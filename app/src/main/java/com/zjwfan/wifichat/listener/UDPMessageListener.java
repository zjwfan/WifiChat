package com.zjwfan.wifichat.listener;
...
import com.zjwfan.wifichat.interfaces.OnUDPReceiveMessage;
...
public class UDPMessageListener extends UDPListener {
		...
		try {
			...
			switch (type) {
				case ADD_USER:// 增加一个用户
					...
					// 构造回送报文内容
					if (!WifiChatApplication.mainInstance.getLocalIp().equals(user.getIp())) {
						users.put(sourceIp, user);
						send(WifiChatApplication.mainInstance.getMyUdpMessage("", LOGIN_SUCC).toString(),
								packet.getAddress());
					}
					break;
				...
				case HEART_BEAT:// 心跳包检测
					send(WifiChatApplication.mainInstance.getMyUdpMessage("", HEART_BEAT_REPLY).toString(),
							packet.getAddress());// 回复心跳包
				...
				case REQUIRE_ICON:// 请求头像
					File file = new File(WifiChatApplication.iconPath + SettingsActivity.iconName);
					if (file.exists()) {
						...
						listener.sendFile(sourceIp, file, WifiChatApplication.iconPath,
	...
	void noticeOffline() {
		try {
			send(WifiChatApplication.mainInstance.getMyUdpMessage("", REMOVE_USER).toString(),
	...
	public void noticeOnline() {
		try {
			...
			if (WifiChatApplication.mainInstance == null) {

			} else {
				send(WifiChatApplication.mainInstance.getMyUdpMessage("", ADD_USER).toString(),
						InetAddress.getByName(Constant.ALL_ADDRESS));
			}
