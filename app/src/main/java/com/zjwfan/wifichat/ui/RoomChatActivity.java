...
import com.zjwfan.wifichat.adapter.RoomChatMsgAdapter;
 ...
 * @creation 2013-6-4
 */
public class RoomChatActivity extends BaseActivity implements OnClickListener, FaceDialog.FaceSelect {
    ...
    private void sendMsg(UDPMessage msg) {
        if (binder != null) {
            ...
            Intent intent = new Intent(RoomChatActivity.this, ChatService.class);
            bindService(intent, connection = new MyServiceConnection(), Context.BIND_AUTO_CREATE);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.room_chat_send:
                ...
                if ("".equals(msg))
                    return;
                UDPMessage message = WifiChatApplication.mainInstance.getMyUdpMessage(msg, Listener.TO_ALL_MESSAGE);
                sendMsg(message);
                break;
